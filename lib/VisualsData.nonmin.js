    ...
    357: function(module, exports) {
        ...
        !function(powerbi) {
            function createSolidFillDefinition(color) {
                return {
                    solid: {
                        color: SQExprBuilder.text(color)
                    }
                };
            }
            var SQExprBuilder = powerbi.data.SQExprBuilder;
            powerbi.createSolidFillDefinition = createSolidFillDefinition;
            ...
            !function(data) {
                ...
                !function(DataViewTransform) {
                    ...
                    function transformDataView(prototype, objectDescriptors, roleMappings, transforms, colorAllocatorFactory, dataRoles, selectsToInclude) {
                        ...
                        return transformed = transformSelects(transformed, targetKinds, roleMappings, transforms.selects, projectionOrdering), 
                        transformObjects(transformed, targetKinds, objectDescriptors, transforms.objects, transforms.selects, colorAllocatorFactory), 
                        data.DataViewRemoveSelects.apply(transformed, targetKinds, selectsToInclude), transformed = data.DataViewConcatenateCategoricalColumns.detectAndApply(transformed, objectDescriptors, roleMappings, projectionOrdering, transforms.selects, projectionActiveItems), 
                    ...
                    function findSelectorsForRuleInput(dataView, selectorRoles) {
                        var dataViewCategorical = dataView.categorical;
                        if (dataViewCategorical) {
                            for (var categoryRoles, categories = dataViewCategorical.categories, selectorsForRuleInput = [], categoryIndex = 0, categoryCount = categories.length; categoryCount > categoryIndex; categoryIndex++) {
                                ...
                                categoryRoles && categoryIdentityFields && columnHasSelectorRole && selectorsForRuleInput.push({
